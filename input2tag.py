#!/usr/bin/env python3
import argparse
from lib.common import AppException,InputBase,format_timedelta
    ...
    def out_file(self):
        ...
        with open(args.out_file,"w",encoding='utf-8') as f:
            for data in self.in_data_arr:
                lines.append( f"[{format_timedelta(data.start)}.000]{data.org}")
                lines.append( f"[{format_timedelta(data.start)}.000]{data.jp}")
            f.write("\n".join(lines))
